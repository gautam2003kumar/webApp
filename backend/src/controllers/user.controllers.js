...
const updateUserCoverImage = asyncHandler(async (req, res) =>{
    ...
    const userWithOldCoverImage = await User.findById(req.user._id).select("coverImage")

    const oldCoverImageUrl = userWithOldCoverImage?.coverImage
...
const getUserChannelProfile = asyncHandler(async(req, res) => {
    ...
    return res
        .status(200)
        .json(
            new ApiResponse(200, chennel[0], "User channel fatched sucessfully" )
        )
})
...
const getWatchHistory = asyncHandler(async (req, res) => {
        ...
        },
        {
            $lookup: {
                ...
                pipeline: [
                    {
                        $lookup: {
                            from: "users",  // should be "users" for User collection
                            localField: "owner",
                            foreignField: "_id",
                            as: "owner",
                            pipeline: [
    ...
    if (!user || !user.length || !user[0].watchHistory) {
    ...
    return res.status(200).json(
        new ApiResponse(
            200,
            user[0].watchHistory,  // Corrected from watchHIstory
